...
export default function handler(req, res) {
  ...
  try {
    ...
    if (route.startsWith('categories/')) {
      const slug = route.split('categories/')[1];
    ...
    // Featured helplines
    if (route === 'helplines/featured') {
      return res.json(helplines.filter(h => h.isFeatured));
    }

    // Helplines by category
    if (route.startsWith('helplines/category/')) {
      const slug = route.split('helplines/category/')[1];
      const category = categories.find(c => c.slug === slug);
      if (!category) {
        return res.status(404).json({ error: 'Category not found' });
      }
      return res.json(helplines.filter(h => h.categoryId === category.id));
    }

    // Search helplines
    if (route.startsWith('helplines/search/')) {
      const query = decodeURIComponent(route.split('helplines/search/')[1]).toLowerCase();
